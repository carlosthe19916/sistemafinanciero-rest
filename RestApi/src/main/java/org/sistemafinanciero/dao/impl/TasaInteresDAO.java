...
public class TasaInteresDAO implements DAO<Object, TasaInteres> {
	...
	public List<TasaInteres> findRange(int[] range) {
    ...
    public int count() {
        javax.persistence.criteria.CriteriaQuery cq = this.em.getCriteriaBuilder().createQuery();
        javax.persistence.criteria.Root<TasaInteres> rt = cq.from(TasaInteres.class);
	...
	public List<TasaInteres> findByNamedQuery(String queryName,
			int resultLimit) {
	...
	public List<TasaInteres> findByNamedQuery(String namedQueryName, Map<String, Object> parameters, Integer offset, Integer limit) {
		Set<Entry<String, Object>> rawParameters = parameters.entrySet();
		Query query = this.em.createNamedQuery(namedQueryName);
		for (Entry<String, Object> entry : rawParameters) {
			query.setParameter(entry.getKey(), entry.getValue());
