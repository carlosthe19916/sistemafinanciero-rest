...
public class ValorTasaInteresDAO implements DAO<Object, ValorTasaInteres> {
	...
	public List<ValorTasaInteres> findRange(int[] range) {
    ...
    public int count() {
        javax.persistence.criteria.CriteriaQuery cq = this.em.getCriteriaBuilder().createQuery();
        javax.persistence.criteria.Root<ValorTasaInteres> rt = cq.from(ValorTasaInteres.class);
	...
	public List<ValorTasaInteres> findByNamedQuery(String namedQueryName, Map<String, Object> parameters, Integer offset, Integer limit) {
		Set<Entry<String, Object>> rawParameters = parameters.entrySet();
		Query query = this.em.createNamedQuery(namedQueryName);
		for (Entry<String, Object> entry : rawParameters) {
			query.setParameter(entry.getKey(), entry.getValue());
