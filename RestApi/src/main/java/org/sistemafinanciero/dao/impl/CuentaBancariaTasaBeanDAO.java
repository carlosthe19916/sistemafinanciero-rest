...
public class CuentaBancariaTasaBeanDAO implements DAO<Object, CuentaBancariaTasa> {

	@PersistenceContext
	private EntityManager em;

	public CuentaBancariaTasa create(CuentaBancariaTasa t) {
		this.em.persist(t);
		return t;
	}

	public void delete(CuentaBancariaTasa t) {
		t = this.em.merge(t);
		this.em.remove(t);
	}

	public CuentaBancariaTasa find(Object id) {
		return this.em.find(CuentaBancariaTasa.class, id);
	}

	public CuentaBancariaTasa update(CuentaBancariaTasa t) {
		return this.em.merge(t);
	}

	public List<CuentaBancariaTasa> findAll() {
		List<CuentaBancariaTasa> list = null;
		CriteriaQuery cq = this.em.getCriteriaBuilder().createQuery();
		cq.select(cq.from(CuentaBancariaTasa.class));
		list = this.em.createQuery(cq).getResultList();
		return list;
	}
	
	public List<CuentaBancariaTasa> findRange(int[] range) {
    ...
    public int count() {
        javax.persistence.criteria.CriteriaQuery cq = this.em.getCriteriaBuilder().createQuery();
        javax.persistence.criteria.Root<CuentaBancariaTasa> rt = cq.from(CuentaBancariaTasa.class);
	...
	public List<CuentaBancariaTasa> findByNamedQuery(String queryName,
			int resultLimit) {
