...
public class CuentaBancariaBeanDAO implements DAO<Object, CuentaBancaria> {

	@PersistenceContext
	private EntityManager em;

	public CuentaBancaria create(CuentaBancaria t) {
		this.em.persist(t);
		return t;
	}

	public void delete(CuentaBancaria t) {
		t = this.em.merge(t);
		this.em.remove(t);
	}
	...
	public CuentaBancaria update(CuentaBancaria t) {
    ...
    public int count() {
        javax.persistence.criteria.CriteriaQuery cq = this.em.getCriteriaBuilder().createQuery();
        javax.persistence.criteria.Root<CuentaBancaria> rt = cq.from(CuentaBancaria.class);
        cq.select(this.em.getCriteriaBuilder().count(rt));
        javax.persistence.Query q = this.em.createQuery(cq);
        return ((Long) q.getSingleResult()).intValue();
    }
    
	public List<CuentaBancaria> findByNamedQuery(String namedQueryName) {
	...
	public List<CuentaBancaria> findByNamedQuery(String queryName,
			int resultLimit) {
	...
	public List<CuentaBancaria> findByNamedQuery(String namedQueryName, Map<String, Object> parameters, Integer offset, Integer limit) {
		Set<Entry<String, Object>> rawParameters = parameters.entrySet();
		Query query = this.em.createNamedQuery(namedQueryName);
		for (Entry<String, Object> entry : rawParameters) {
			query.setParameter(entry.getKey(), entry.getValue());
