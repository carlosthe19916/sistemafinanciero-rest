...
public class TrabajadorBeanDAO implements DAO<Object, Trabajador> {
	...
	public Trabajador find(Object id) {
		return this.em.find(Trabajador.class, id);
	}
	...
	public List<Trabajador> findRange(int[] range) {
    ...
    public int count() {
        javax.persistence.criteria.CriteriaQuery cq = this.em.getCriteriaBuilder().createQuery();
        javax.persistence.criteria.Root<Trabajador> rt = cq.from(Trabajador.class);
        cq.select(this.em.getCriteriaBuilder().count(rt));
        javax.persistence.Query q = this.em.createQuery(cq);
        return ((Long) q.getSingleResult()).intValue();
    }
    
	public List<Trabajador> findByNamedQuery(String namedQueryName) {
	...
	public List<Trabajador> findByNamedQuery(String queryName,
			int resultLimit) {
	...
	public List<Trabajador> findByNamedQuery(String namedQueryName, Map<String, Object> parameters, Integer offset, Integer limit) {
		Set<Entry<String, Object>> rawParameters = parameters.entrySet();
		Query query = this.em.createNamedQuery(namedQueryName);
		for (Entry<String, Object> entry : rawParameters) {
			query.setParameter(entry.getKey(), entry.getValue());
