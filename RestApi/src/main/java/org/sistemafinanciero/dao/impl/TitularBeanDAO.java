...
public class TitularBeanDAO implements DAO<Object, Titular> {

	@PersistenceContext
	private EntityManager em;

	public Titular create(Titular t) {
		this.em.persist(t);
		return t;
	}
	...
	public Titular find(Object id) {
		return this.em.find(Titular.class, id);
	}

	public Titular update(Titular t) {
	...
	public List<Titular> findRange(int[] range) {
    ...
    public int count() {
        javax.persistence.criteria.CriteriaQuery cq = this.em.getCriteriaBuilder().createQuery();
        javax.persistence.criteria.Root<Titular> rt = cq.from(Titular.class);
        cq.select(this.em.getCriteriaBuilder().count(rt));
        javax.persistence.Query q = this.em.createQuery(cq);
        return ((Long) q.getSingleResult()).intValue();
    }
    
	public List<Titular> findByNamedQuery(String namedQueryName) {
	...
	public List<Titular> findByNamedQuery(String queryName,
			int resultLimit) {
	...
	public List<Titular> findByNamedQuery(String namedQueryName, Map<String, Object> parameters, Integer offset, Integer limit) {
		Set<Entry<String, Object>> rawParameters = parameters.entrySet();
		Query query = this.em.createNamedQuery(namedQueryName);
		for (Entry<String, Object> entry : rawParameters) {
			query.setParameter(entry.getKey(), entry.getValue());
