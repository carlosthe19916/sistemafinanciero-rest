...
public class BeneficiarioBeanDAO implements DAO<Object, Beneficiario> {
	...
	public void delete(Beneficiario t) {
		t = this.em.merge(t);
		this.em.remove(t);
	}

	public Beneficiario find(Object id) {
		return this.em.find(Beneficiario.class, id);
	}
	...
	public List<Beneficiario> findRange(int[] range) {
    ...
    public int count() {
        javax.persistence.criteria.CriteriaQuery cq = this.em.getCriteriaBuilder().createQuery();
        javax.persistence.criteria.Root<Beneficiario> rt = cq.from(Beneficiario.class);
        cq.select(this.em.getCriteriaBuilder().count(rt));
        javax.persistence.Query q = this.em.createQuery(cq);
        return ((Long) q.getSingleResult()).intValue();
    }
    
	public List<Beneficiario> findByNamedQuery(String namedQueryName) {
	...
	public List<Beneficiario> findByNamedQuery(String queryName,
			int resultLimit) {
	...
	public List<Beneficiario> findByNamedQuery(String namedQueryName, Map<String, Object> parameters, Integer offset, Integer limit) {
		Set<Entry<String, Object>> rawParameters = parameters.entrySet();
		Query query = this.em.createNamedQuery(namedQueryName);
		for (Entry<String, Object> entry : rawParameters) {
			query.setParameter(entry.getKey(), entry.getValue());
