...
public class SocioViewBeanDAO implements DAO<Object, SocioView> {

	@PersistenceContext
	private EntityManager em;

	public SocioView create(SocioView t) {
		this.em.persist(t);
		return t;
	}

	public void delete(SocioView t) {
		t = this.em.merge(t);
		this.em.remove(t);
	}

	public SocioView find(Object id) {
		return this.em.find(SocioView.class, id);
	}

	public SocioView update(SocioView t) {
	...
	public List<SocioView> findAll() {
		...
		cq.select(cq.from(SocioView.class));
		list = this.em.createQuery(cq).getResultList();
		return list;
	}
	
	public List<SocioView> findRange(int[] range) {
    ...
    public int count() {
        javax.persistence.criteria.CriteriaQuery cq = this.em.getCriteriaBuilder().createQuery();
        javax.persistence.criteria.Root<SocioView> rt = cq.from(SocioView.class);
	...
	public List<SocioView> findByNamedQuery(String queryName,
			int resultLimit) {
