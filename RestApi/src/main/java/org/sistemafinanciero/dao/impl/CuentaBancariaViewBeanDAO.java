...
public class CuentaBancariaViewBeanDAO implements DAO<Object, CuentaBancariaView> {

	@PersistenceContext
	private EntityManager em;

	public CuentaBancariaView create(CuentaBancariaView t) {
		this.em.persist(t);
		return t;
	}

	public void delete(CuentaBancariaView t) {
		t = this.em.merge(t);
		this.em.remove(t);
	}

	public CuentaBancariaView find(Object id) {
		return this.em.find(CuentaBancariaView.class, id);
	}

	public CuentaBancariaView update(CuentaBancariaView t) {
	...
	public List<CuentaBancariaView> findAll() {
		...
		cq.select(cq.from(CuentaBancariaView.class));
		list = this.em.createQuery(cq).getResultList();
		return list;
	}
	
	public List<CuentaBancariaView> findRange(int[] range) {
    ...
    public int count() {
        javax.persistence.criteria.CriteriaQuery cq = this.em.getCriteriaBuilder().createQuery();
        javax.persistence.criteria.Root<CuentaBancariaView> rt = cq.from(CuentaBancariaView.class);
        cq.select(this.em.getCriteriaBuilder().count(rt));
        javax.persistence.Query q = this.em.createQuery(cq);
        return ((Long) q.getSingleResult()).intValue();
    }
    
	public List<CuentaBancariaView> findByNamedQuery(String namedQueryName) {
	...
	public List<CuentaBancariaView> findByNamedQuery(String queryName,
			int resultLimit) {
	...
	public List<CuentaBancariaView> findByNamedQuery(String namedQueryName, Map<String, Object> parameters, Integer offset, Integer limit) {
		Set<Entry<String, Object>> rawParameters = parameters.entrySet();
		Query query = this.em.createNamedQuery(namedQueryName);
		for (Entry<String, Object> entry : rawParameters) {
			query.setParameter(entry.getKey(), entry.getValue());
