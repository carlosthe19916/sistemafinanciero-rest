...
public class HistorialCajaBeanDAO implements DAO<Object, HistorialCaja> {

	@PersistenceContext
	private EntityManager em;
	
	public HistorialCaja create(HistorialCaja t) {
		this.em.persist(t);
		return t;
	}

	public void delete(HistorialCaja t) {
		t = this.em.merge(t);
		this.em.remove(t);
	}

	public HistorialCaja find(Object id) {
		return this.em.find(HistorialCaja.class, id);
	}

	public HistorialCaja update(HistorialCaja t) {
		return this.em.merge(t);
	}

	public List<HistorialCaja> findAll() {
		List<HistorialCaja> list = null;
		CriteriaQuery cq = this.em.getCriteriaBuilder().createQuery();
		cq.select(cq.from(HistorialCaja.class));
		list = this.em.createQuery(cq).getResultList();
		return list;
	}
	
	public List<HistorialCaja> findRange(int[] range) {
    ...
    public int count() {
        javax.persistence.criteria.CriteriaQuery cq = this.em.getCriteriaBuilder().createQuery();
        javax.persistence.criteria.Root<HistorialCaja> rt = cq.from(HistorialCaja.class);
	...
	public List<HistorialCaja> findByNamedQuery(String queryName,
			int resultLimit) {
